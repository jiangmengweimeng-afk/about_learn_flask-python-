...
PyJWT 和 Flask-JWT-Extended 的区别 我们可从下面几个方面考虑：
上手难度：pJ相对来说是较低的 接近底层，而FJE非常简单并且封装度高
代码量： pJ需要手动编写装饰器和异常处理，而FJE使用@jwt_required()一行就行了
获取参数： pay = jwt.decode()后从字典里面取值， 而FJE使用get_jwt_identity()或者get_jwt()

回滚是数据库事务中的一个重要概念，指的是取消当前事务中所有未提交的更改，将数据库恢复到事务开始前的状态 确保了事务的 原子性：事务要么全部成功，要么全部失败
                一致性：事务前后数据库状态保持正确
                隔离性：并发事务互不干扰
                持久性：提交后更改永久保存

### ORM
- ORM就是对象关系映射，是一种编程技术，用于在关系型数据库和面向对象编程语言之间建立桥梁 如图所示
  ![alt text](imgs/learn_solve_token.image.png)
- 优点：提高开发效率，用面向对象思维操作数据库
    - 代码可读性更清晰简洁
    - 数据库无关性，更换数据库改动小
    - 防止SQL注入，自动参数化查询
    - 简化关系处理，轻松处理外键关联查询
- 缺点：性能开销比原生SQL慢
    - 复杂查询困难，非常复杂的查询仍然需要写SQL
    - 需要学习ORM框架
    - 调试困难生成的SQL可能不够优化
