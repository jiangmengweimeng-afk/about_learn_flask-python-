...
## 可变和不可变
**可变对象**和**不可变对象**的核心区别在于**对象创建后，其内部状态是否能被修改**

### 可变： 
- List：
  - 有序性
  - 灵活性：列表中的元素是按照插入顺序存储的，可以索引、切片、追加、插入、删除等
- Dict:
...
判断是否可变或者不可变的方法：
- 用`id()`查看对象地址，修改过后地址不改变为可变；地址改变为不可变。
- 用`hash()`不可变对象通常可以哈希作为字典键，可变对象不可哈希，会报错。

---

### 装饰器
- **语法糖：** python提供了一种简洁的语法来应用装饰器，使用`@decorator_name`语法开装饰函数
  
装饰器不是被“调用”的，而是被**应用** 或**绑定** 到目标函数上的,而且装饰器在**定义阶段**执行替换，在**运行阶段**执行包装函数。

**python中调用和应用的区别**
- **调用：** 以为着执行代码`decorator()`，会产生返回值，正确的说法是**应用**或**绑定**
- **应用：** 意味着语法糖`@decorator`在**编译/定义时** 将目标函数作为参数传入，并**替换** 掉原函数

**关于我是如何理解装饰器是封装功能的专业术语的解释:**

- **装饰器**是一个**高阶函数，** 它接收一个**可调用对象**作为参数，并返回一个新的可调用对象。通过`@`语法糖，装饰器在**编译**阶段被**应用**与目标函数，将目标函数**替换**为装饰器返回的**包装函数**。其本质是实现**面向切面编程**，用于在不修改原函数源代码的情况下，**织入**额外的横切逻辑。
...
### 关于封装我理解错的地方
**封装**是隐藏数据，装饰器是增强行为，正确的说法是**增强**或**包裹**

### python中的多线程编程，优势和限制
python中多线程编程是通过并发执行多个线程来提高程序性能的一种技术。每个线程都是独立执行单元，使得程序能够在单个进程中并行执行多个任务

**优势：**
- **提高性能：** 对于I/O密集型任务（文件读写、网络i请求），多线程能够是的程序在等待I/O时可切换到其他线程执行，提高效率
- **更好的响应性：** 对于同时需要处理多个任务的应用程序，多线程可以时程序更加响应迅速
- **资源共享：** 多个线程共享同一进程的资源，入内存空间，可有效减少资源的浪费

**限制：**
- **GIL：** python中的GIL会限制在同一时刻只有一个线程能够执行python字节码，在CPU密集型任务中，多线程并不能实现真正的并行，仅能在I/O阻塞时切换执行
- **线程安全：** 多线程共享内存，可能会导致数据竞争、死锁等问题，需要额外的同步和锁机制来保证线程的安全
- **C扩展的限制：** GIL对C扩展也有限制，无法解决python GIL带来的多线程并发问题

### 迭代器和生成器之间的区别
**迭代器：**
- **定义：** 迭代器是一种对象，能够表示一系列数据，通常用于迭代集合、列表和其他可迭代对象
- **工作方式：** 通过`iter()`函数和`next()`方法，一次遍历数据流中的元素。`iter()`用于生成一个爹大气对象, `next()`则从该对象中获取下一个元素
- **内存占用：** 逐个产生数据，因此可以处理大型数据集而无需占用大量内存
- **实现：** `__iter__()` `__next__()`
  ![alt text](imgs/common_mistakes.image-10.png)

**生成器：**
- **定义：** 生成器是一种特殊的迭代器，使用`yield`语句按需生成数据
- **工作方式：** 当调用生成器函数时，不执行，而是返回一个生成器对象。每次调用`next()`方法时，它会执行生成器函数，知道遇到`yield`语句返回结果，并在此处暂停，等待下一次调用
- **内存占用：** 与迭代器类似，它逐个产生数据，因此节省内存
- **实现：** 函数定义和`yield`语句创建
  ![alt text](imgs/common_mistakes.image-11.png)