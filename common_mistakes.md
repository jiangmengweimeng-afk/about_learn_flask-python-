...
### python语法规则（非FastAPI 特有）
| 在python中，带默认值的参数之后不能出现不带默认值的参数（仅限位置参数）

- 如果我将没有默认值的参数写在有默认值参数的后面，则会报错`SyntaxError: non-default argument follows default argument`
- 但是如果所有的参数都有默认值，**Python不会报语法错误，** 但是FastAPI在解析时候可能因参数来源冲突而报逻辑错误。

### 正确写法标准模板

**将依赖项(Depends)放在所有参数之后，原因如下：**
- 依赖项通常与业务逻辑有关
- 查询/路径参数更加直观，放在前面便于阅读
- 避免误以为顺序会影响功能
![alt text](imgs/common_mistakes.image-9.png)

### 关于HTTP方法和资源URL

这些路由方法不同，是因为他们遵循**RESTful 设计规范，** 通过组合**HTTP 方法** 和 **资源URL 结构** 来表达对不同资源的操作语义：
- `GET/` 是**集合查询，** 使用查询参数进行过滤和分页
- `POST/` 是**集合创建，** 通过请求传递新资源数据
- `PUT/{record_id}` 是**实力更新，** 通过路径参数定位资源，请求体传递更新字段

这种设计保证了接口的**幂等性、** **安全性** 和**可缓存性** 符合HTTP标准，同时提升API的可读性和可维护性 