...
| Python代码 | SQL关键字 | 属于 |
|---|---|---|
|`.where()` | `WHERE` | **筛选条件（过滤行）**|
|`order_by()` | `ORDER BY` | **排序规则（排列顺序）**|
|`.limit()` | `LIMIT` | **数量限制（取前N条）** |
|`.offset()` | `OFFSET` | **起始位置（跳过前N条）** |

---

### 为什么要构建基础查询呢？
**核心原因：DRY原则，****总记录数查询** 和 **数据列表查询**用的是**完全相同得筛选条件，**  如果不构建基础查询，我就得写两边重复得条件。
- 条件只写一次
- 新增筛选条件时候，只需要在`conditions`列表是加一行
- COUNT和列表数据保证使用完全相同得条件
- 代码更清晰，逻辑结构一目了然

### 什么是in_
`in_`是SQLALchemy列对象上的一个方法，用于生成SQL的`IN`比较表达式。被称为调用ORM操作符方法，它对应SQL标准中的`IN`操作符。

**`in`和`in_`的区别：** 
- `in:` 是python语言本身的成员运算符，用于判断某个对象是否存在于一个集合中
- `in_:` 由于`in`是python的保留关键字，SQLAlchemy无法将其直接用作方法名，因此采用了加下划线的命名规范。调用这个方法时，SQLAlchemy会在内部将其翻译为标准的SQL语法。

---

### ge/le/depends
- `ge=1:`表示这个值**必须** >=1，否则FastAPI会自动返回校验错误
